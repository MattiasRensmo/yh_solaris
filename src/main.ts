...
const domPageHeader = document.querySelector<HTMLElement>('#page-header')
const domSolarSystem = document.querySelector<HTMLElement>('#solar-system')
const domPlanetInfo = document.querySelector<HTMLElement>('#planet-info')
const domFooter = document.querySelector<HTMLElement>('#footer')
const domPlanetName = document.querySelector<HTMLElement>('#planet-name')

// Dom objects on details page
const domDetailsPlanet = document.querySelector<HTMLElement>('#details__planet')
const domDetailsClose = document.querySelector<HTMLElement>('#details__close')
const domDetailsBackground = document.querySelector<HTMLElement>('.details__background')
 ...
 */
async function getApiKey(url: string) {
  try {
...
function setPlanetSize(bodies: Planet[]) {
  ...
  bodies.forEach((planet) => {
    ...
    const domPlanet = document.querySelector<HTMLElement>(`#planet-${id}`)
 ...
 */
function showPlanetName(event: MouseEvent) {
  //No hover on touchscreens
  if (!('ontouchstart' in document.documentElement)) {
    if (domPlanetName) {
      const planet = event.target as HTMLElement
 ...
 */
function addToDom(input: string[] | string | number, unit: string, elementName: string) {
  const parent = document.querySelector<HTMLElement>(elementName)
  if (!parent) return
  if (Array.isArray(input)) {
    const ul = parent //document.querySelector(elementName)

    ul.innerHTML = ''
    const clean: string[] = noDuplicates(input)
...
function toggleDetails(showDetails: boolean) {
  if (
    !domPageHeader ||
    !domSolarSystem ||
    !domPlanetInfo ||
